var date="";
var time="";
var dates=db.JUP_DB_Sales.find({},{last_updated_date:1,last_updated_time:1}).sort({last_updated_date:1,last_updated_time:1}).limit(1)
...
if(date!=null && time!=null){
		...
		var cursor_network=db.JUP_DB_Sales.aggregate([
		{"$match":{
			...
			cursor_network.forEach(function(x){
				...
				cumu_network.forEach(function(y){
					if(combine_column_==y.combine_column){
						x.sale_pax+=y.sale_pax;
						x.sale_pax_1+=y.sale_pax_1;
						x.sale_pax_2+=y.sale_pax_2;
						x.revenue+=y.sale_revenue;
						x.revenue_1+=y.sale_revenue_1;
						x.revenue_2+=y.sale_revenue_2;
						x.revenue_base+=y.sale_revenue_base;
						x.revenue_base_1+=y.sale_revenue_base_1;
						x.revenue_base_2+=y.sale_revenue_base_2;
					}		
				})
				...
				db.getCollection('JUP_DB_Cumulative_Dep_Date').update({
					combine_column:combine_column_ 
							...
							channel:x.channel,
							sale_pax:x.sale_pax,
							sale_pax_1:x.sale_pax_1,
							sale_pax_2:x.sale_pax_2,
							sale_revenue:x.revenue,
							sale_revenue_1:x.revenue_1,
							sale_revenue_2:x.revenue_2,
							sale_revenue_base:x.revenue_base,
							sale_revenue_base_1:x.revenue_base_1,
							sale_revenue_base_2:x.revenue_base_2
		...
		var cursor_region=db.JUP_DB_Sales.aggregate([
		{"$match":{
			"last_updated_date" : {'$eq':date},
			"last_updated_time" : {'$eq':time},
			"region" : {'$ne':null}
		...
		var cursor_country=db.JUP_DB_Sales.aggregate([
		{"$match":{
			"last_updated_date" : {'$eq':date},
			"last_updated_time" : {'$eq':time},
			"country" : {'$ne':null}
		...
		// pos level access
		var cursor_pos=db.JUP_DB_Sales.aggregate([
		{"$match":{
			"last_updated_date" : {'$eq':date},
			"last_updated_time" : {'$eq':time},
			"pos" : {'$ne':null}
