...
data.this_month.dep_date.forEach(function(x){
    if(new Date(x.dep_date) > yesterDay){
        ...
        x['yield'] = x.distance_flown == 0 ? 0 : (x.flown_revenue/(x.flown_pax*x.distance_flown))*100;
    }
...
_distance_1 = data.this_month.distance_1;
...
var _yieldVLYR = (yield_(_saleRev,_MonthPaxDist)-yield_(_saleRev_1,_MonthPaxDist_1))/(yield_(_saleRev_1, _MonthPaxDist_1)==0?1:yield_(_saleRev_1, _MonthPaxDist_1))*100;
...
// assigning KPI value  
_KPIObj['Revenue'] ={'current':_saleRev,'target':_TargetRevenue} ;
